...
function checkSpec(where, spec, { trigger, target }) {
  ...
  for (const s of steps) {
    ...
    if (!op.async && !op.self && !s.to) err(`${at}: needs "to"`);
...
function checkAttack(where, a, duo) {
  if (!a || !STYLES[a.style]) err(`${where}: unknown attack style "${a && a.style}"`);
  if (!a.name && !where.includes('dummy')) warn(`${where}: attack has no name`);
  if (!/^#[0-9a-f]{3,8}$/i.test(a.color || '')) err(`${where}: bad attack color`);
}

const TRIGGERS = ['onPlay', 'onDeath', 'onTurnStart', 'onAllyDeath', 'onHurt'];
for (const [id, c] of Object.entries(MB.CARDS)) {
  ...
  if (c.intro) {
    if (typeof c.intro === 'string') { if (!INTRO[c.intro]) err(`${at}: unknown intro "${c.intro}"`); }
    else {
