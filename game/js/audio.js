...
(function () {
  ...
  const SAMPLES = {
    ...
    vortex: { file: 'vortex-697634.mp3', vol: 0.6 },
    missile: { file: 'missile-521377.mp3', vol: 0.5 },
    vines: { file: 'vines-677814.mp3', vol: 0.7 },
    rune: { file: 'rune-786290.mp3', vol: 0.6, max: 1.6 },
    shuriken: { file: 'shuriken-196563.mp3', vol: 0.6, vary: 0.1 },
    melody: { file: 'melody-70175.mp3', vol: 0.5, max: 2.2 },
    poof: { file: 'poof-208111.mp3', vol: 0.6, vary: 0.08 },
    gunshot: { file: 'gunshot-632821.mp3', vol: 0.45, max: 1.2 },
    scope: { file: 'scope-210018.mp3', vol: 0.6 },
    lava: { file: 'lava-361211.mp3', vol: 0.5 },
    kiss: { file: 'kiss-346661.mp3', vol: 0.8 },
    swish: { file: 'swish-475132.mp3', vol: 0.7, vary: 0.1 },
    clang: { file: 'clang-326795.mp3', vol: 0.45, vary: 0.08 },
    glitch: { file: 'glitch-348585.mp3', vol: 0.5 },
    camera: { file: 'camera-249750.mp3', vol: 0.7 },
    ...
    guitar: { file: 'guitar-104622.mp3', vol: 0.55, max: 2 },
    piano: { file: 'piano-508804.mp3', vol: 0.6, max: 2.2 },
    axe: { file: 'axe-452554.mp3', vol: 0.8, vary: 0.06 },
    flip: { file: 'flip-248143.mp3', vol: 0.6 },
    slide: { file: 'slide-489729.mp3', vol: 0.7, max: 1.2 },
    glass: { file: 'glass-221528.mp3', vol: 0.45, max: 1.4, vary: 0.06 },
    flutter: { file: 'flutter-561009.mp3', vol: 0.6, max: 1.6, vary: 0.08 },
    stomp: { file: 'stomp-721601.mp3', vol: 0.8, vary: 0.05 },
    grow: { file: 'grow-607409.mp3', vol: 0.5 },
    trash: { file: 'trash-498434.mp3', vol: 0.5, max: 1.6 },
    drumroll: { file: 'drumroll-197763.mp3', vol: 0.5 },
    gong: { file: 'gong-82523.mp3', vol: 0.45 },
    sizzle: { file: 'sizzle-540080.mp3', vol: 0.5, max: 2 },
    applause: { file: 'applause-245639.mp3', vol: 0.45, max: 2.6 },
    hypno: { file: 'hypno-592950.mp3', vol: 0.55 },
  ...
  const SFX = {
    ...
    wind: () => SFX.whoosh(),
    boom: () => SFX.slam(),
    // synth stand-ins for the attack samples
    choir: () => SFX.holy(),
    laugh: () => [0, 0.14, 0.28, 0.42].forEach((d, i) => tone({ type: 'triangle', f0: 520 - i * 30, f1: 380 - i * 30, dur: 0.1, vol: 0.1, delay: d })),
    incoming: () => SFX.whistleDown(),
    tornado: () => noise({ dur: 1.2, vol: 0.3, f: 300, sweep: 1500, q: 1.5 }),
    vortex: () => { SFX.dark(); SFX.whoosh(); },
    missile: () => noise({ dur: 0.7, vol: 0.3, f: 800, sweep: 3000, q: 0.8 }),
    vines: () => SFX.draw(),
    rune: () => SFX.buff(),
    shuriken: () => SFX.zip(),
    melody: () => [784, 988, 1175, 988, 1319].forEach((f, i) => tone({ type: 'sine', f0: f, dur: 0.3, vol: 0.08, delay: i * 0.15 })),
    poof: () => noise({ dur: 0.35, vol: 0.3, f: 900, sweep: 250, q: 0.7 }),
    gunshot: () => { noise({ dur: 0.3, vol: 0.5, f: 2500, sweep: 300, type: 'lowpass' }); tone({ type: 'square', f0: 200, f1: 50, dur: 0.12, vol: 0.2 }); },
    scope: () => SFX.click(),
    lava: () => { SFX.bubble(); SFX.fire(); },
    kiss: () => tone({ type: 'sine', f0: 1400, f1: 600, dur: 0.08, vol: 0.15 }),
    swish: () => SFX.whoosh(),
    clang: () => [1800, 2650, 3900].forEach((f) => tone({ type: 'triangle', f0: f, dur: 0.5, vol: 0.06 })),
    glitch: () => [0, 0.05, 0.12, 0.2].forEach((d) => tone({ type: 'square', f0: 200 + Math.random() * 1800, dur: 0.04, vol: 0.06, delay: d })),
    camera: () => { SFX.click(); noise({ dur: 0.08, vol: 0.2, f: 4000, q: 2, delay: 0.06 }); },
    tick: () => [0, 0.25, 0.5].forEach((d) => tone({ type: 'square', f0: 2000, dur: 0.02, vol: 0.08, delay: d })),
    stab: () => SFX.hit(true),
    cheer: () => SFX.win(),
    scythe: () => SFX.whoosh(),
    surf: () => SFX.wave(),
    guitar: () => [0, 0.18, 0.36].forEach((d, i) => [82, 123, 165].forEach((f) => tone({ type: 'sawtooth', f0: f * (i === 2 ? 1.5 : 1), dur: 0.2, vol: 0.07, delay: d, lp: 1600 }))),
    piano: () => [262, 330, 392, 523].forEach((f, i) => tone({ type: 'triangle', f0: f, dur: 1, vol: 0.09, delay: i * 0.03 })),
    axe: () => { noise({ dur: 0.18, vol: 0.45, f: 1400, sweep: 300, type: 'lowpass' }); tone({ type: 'square', f0: 180, f1: 60, dur: 0.12, vol: 0.15 }); },
    flip: () => [0, 0.08, 0.16, 0.24].forEach((d, i) => tone({ type: 'sine', f0: 2200 + i * 200, dur: 0.06, vol: 0.06, delay: d })),
    slide: () => noise({ dur: 0.7, vol: 0.15, f: 2500, q: 4 }),
    glass: () => SFX.shatter(),
    flutter: () => [0, 0.1, 0.2, 0.3].forEach((d) => noise({ dur: 0.07, vol: 0.2, f: 900, q: 1, delay: d })),
    stomp: () => SFX.slam(),
    grow: () => tone({ type: 'square', f0: 200, f1: 900, dur: 0.6, vol: 0.08, lp: 2000, vib: { rate: 18, depth: 40 } }),
    trash: () => [1200, 1750, 2600].forEach((f, i) => tone({ type: 'triangle', f0: f, f1: f * 0.9, dur: 0.4, vol: 0.06, delay: i * 0.09 })),
    drumroll: () => { for (let i = 0; i < 16; i++) noise({ dur: 0.05, vol: 0.08 + i * 0.01, f: 1800, q: 0.8, delay: i * 0.06 }); },
    gong: () => [110, 165, 247].forEach((f) => tone({ type: 'sine', f0: f, f1: f * 0.98, dur: 1.6, vol: 0.12, vib: { rate: 5, depth: 2 } })),
    sizzle: () => noise({ dur: 1.2, vol: 0.12, f: 6000, type: 'highpass' }),
    applause: () => { for (let i = 0; i < 24; i++) noise({ dur: 0.04, vol: 0.12, f: 2500, q: 1.5, delay: Math.random() * 1.5 }); },
    hypno: () => tone({ type: 'sine', f0: 500, dur: 1, vol: 0.12, attack: 0.1, vib: { rate: 6, depth: 120 } }),
    warp: () => tone({ type: 'sawtooth', f0: 90, f1: 1600, dur: 0.5, vol: 0.08, lp: 2500, vib: { rate: 30, depth: 50 } }),
    unsheathe: () => { tone({ type: 'sawtooth', f0: 3000, f1: 5000, dur: 0.4, vol: 0.03, lp: 7000 }); noise({ dur: 0.4, vol: 0.12, f: 5000, type: 'highpass' }); },
    stretch: () => tone({ type: 'sawtooth', curve: [120, 180, 140, 200], dur: 0.5, vol: 0.06, lp: 900 }),
    // Nerida's siren song: a high, wavering voice
    siren: () => [880, 1175, 988].forEach((f, i) => tone({ type: 'sine', f0: f, f1: f * 1.02, dur: 0.5, vol: 0.09, attack: 0.12, delay: i * 0.35, vib: { rate: 6, depth: 14 } })),
